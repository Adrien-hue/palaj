# core/domain/entities/work_day.py
...
class WorkDay:
    ...
    def type(self) -> TypeJour:
        ...
        return TypeJour.POSTE if self.tranches else TypeJour.REPOS
    ...
    def _tranche_is_nocturne(self, tranche: Tranche) -> bool:
        ...
        debut, fin = tranche.debut, tranche.fin
    ...
    def start_time(self) -> Optional[time]:
        ...
        if not self.tranches:
            return None
        return min(t.debut for t in self.tranches)

    def end_time(self) -> Optional[time]:
        """Heure de fin de la dernière tranche."""
        if not self.tranches:
            return None
        return max(t.fin for t in self.tranches)
    ...
    def __str__(self) -> str:
        ...
        tranches_str = (
            ", ".join(f"{t.abbr}({t.debut.strftime('%H:%M')}-{t.fin.strftime('%H:%M')})"
                      for t in self.tranches)
