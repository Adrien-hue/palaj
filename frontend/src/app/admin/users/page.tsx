"use client";

import { useCallback, useEffect, useMemo, useRef, useState } from "react";

import type { User } from "@/types";
import { listUsers } from "@/services/users.service";
import { apiFetch } from "@/lib/api/universal";
import { backendPath } from "@/lib/backendPath";

import { useConfirm } from "@/hooks/useConfirm";
import { useUsersCrud } from "@/features/users/useUsersCrud";

import { ClientDataTable } from "@/components/tables/ClientDataTable";
import { getUsersTableColumns } from "@/features/users/users.table-columns";

import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { Switch } from "@/components/ui/switch";
import { Label } from "@/components/ui/label";

import UserDetails from "@/features/users/user.details";
import UserForm, { type UserFormHandle } from "@/features/users/user.form";

type CurrentUser = {
  id: number;
};

async function fetchCurrentUser() {
  return apiFetch<CurrentUser>(backendPath("/auth/me"));
}

export default function UsersPage() {
  const { confirm, ConfirmDialog } = useConfirm();

  const [users, setUsers] = useState<User[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [includeInactive, setIncludeInactive] = useState(false);
  const [currentUserId, setCurrentUserId] = useState<number | null>(null);

  const refreshRef = useRef<null | (() => void)>(null);
  const refresh = useCallback(() => refreshRef.current?.(), []);

  const crud = useUsersCrud({ confirm, refresh, setUsers, includeInactive, currentUserId });

  const formRef = useRef<UserFormHandle | null>(null);

  useEffect(() => {
    let cancelled = false;

    async function run() {
      setLoading(true);
      setError(null);
      try {
        const [allUsers, me] = await Promise.all([
          listUsers({ include_inactive: includeInactive }),
          fetchCurrentUser(),
        ]);

        if (!cancelled) {
          setUsers(allUsers);
          setCurrentUserId(me.id);
        }
      } catch (e) {
        if (!cancelled) setError(e instanceof Error ? e.message : "Erreur inconnue");
      } finally {
        if (!cancelled) setLoading(false);
      }
    }

    refreshRef.current = run;
    run();

    return () => {
      cancelled = true;
    };
  }, [includeInactive]);

  const sortedUsers = useMemo(
    () => [...users].sort((a, b) => a.username.localeCompare(b.username, "fr", { sensitivity: "base" })),
    [users]
  );

  const columns = useMemo(
    () =>
      getUsersTableColumns({
        onView: crud.openView,
        onEdit: crud.openEdit,
        onDelete: crud.deleteUser,
        onToggleActive: crud.toggleActive,
        togglingIds: crud.togglingIds,
        currentUserId,
      }),
    [crud, currentUserId]
  );

  const handleEditFromDetails = useCallback(() => {
    if (!crud.detailsUser) return;
    const user = crud.detailsUser;
    crud.closeView();
    crud.openEdit(user);
  }, [crud]);

  return (
    <>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <h1 className="truncate text-2xl font-semibold tracking-tight">Utilisateurs</h1>
          <p className="mt-1 text-sm text-muted-foreground">Gestion des comptes utilisateurs</p>
        </div>

        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <Switch
              id="include-inactive-users"
              checked={includeInactive}
              onCheckedChange={setIncludeInactive}
              disabled={loading}
            />
            <Label htmlFor="include-inactive-users" className="text-sm font-normal">
              Afficher les inactifs
            </Label>
          </div>

          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={refresh} disabled={loading}>
              Actualiser
            </Button>
            <Button onClick={crud.openCreate}>Créer</Button>
          </div>
        </div>
      </div>

      <div className="mt-4">
        {loading ? (
          <Card>
            <CardHeader className="space-y-2">
              <Skeleton className="h-5 w-40" />
              <Skeleton className="h-4 w-72" />
            </CardHeader>
            <CardContent className="space-y-3">
              <Skeleton className="h-10 w-full" />
              <Skeleton className="h-10 w-full" />
              <Skeleton className="h-10 w-full" />
              <Skeleton className="h-10 w-full" />
            </CardContent>
          </Card>
        ) : error ? (
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Erreur</CardTitle>
              <CardDescription className="text-destructive">{error}</CardDescription>
            </CardHeader>
            <CardContent>
              <Button onClick={refresh}>Réessayer</Button>
            </CardContent>
          </Card>
        ) : users.length === 0 ? (
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Aucun utilisateur</CardTitle>
              <CardDescription>Commence par créer ton premier utilisateur.</CardDescription>
            </CardHeader>
            <CardContent>
              <Button onClick={crud.openCreate}>Créer un utilisateur</Button>
            </CardContent>
          </Card>
        ) : (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">Liste des utilisateurs</CardTitle>
              <CardDescription>Recherche, tri, pagination et colonnes configurables.</CardDescription>
            </CardHeader>

            <CardContent className="p-0">
              <div className="p-4 pt-0">
                <ClientDataTable<User>
                  data={sortedUsers}
                  columns={columns}
                  searchPlaceholder="Rechercher un utilisateur…"
                  initialPageSize={20}
                  maxHeightClassName="max-h-[calc(100dvh-360px)]"
                  enableColumnVisibility={false}
                />
              </div>
            </CardContent>
          </Card>
        )}
      </div>

      <ConfirmDialog />

      <Dialog open={crud.detailsOpen} onOpenChange={(v) => !v && crud.closeView()}>
        <DialogContent className="sm:max-w-2xl p-0">
          <DialogHeader className="px-6 pt-6">
            <DialogTitle>Détail utilisateur</DialogTitle>
          </DialogHeader>

          <div className="px-6 py-4 max-h-[70dvh] overflow-auto">
            {crud.detailsUser ? (
              <UserDetails user={crud.detailsUser} />
            ) : (
              <div className="text-sm text-muted-foreground">Aucune donnée.</div>
            )}
          </div>

          <div className="flex items-center justify-end gap-2 border-t bg-background px-6 py-4">
            <Button variant="outline" type="button" onClick={crud.closeView}>
              Fermer
            </Button>
            <Button type="button" onClick={handleEditFromDetails} disabled={!crud.detailsUser}>
              Éditer
            </Button>
          </div>
        </DialogContent>
      </Dialog>

      <Dialog
        open={crud.modalOpen}
        onOpenChange={(v) => {
          if (!v && !crud.submitting) crud.closeModal();
        }}
      >
        <DialogContent
          className="sm:max-w-xl p-0"
          onPointerDownOutside={(e) => crud.submitting && e.preventDefault()}
          onEscapeKeyDown={(e) => crud.submitting && e.preventDefault()}
        >
          <DialogHeader className="px-6 pt-6">
            <DialogTitle>
              {crud.modalMode === "create" ? "Créer un utilisateur" : "Modifier l’utilisateur"}
            </DialogTitle>
          </DialogHeader>

          <div className="px-6 py-4 max-h-[70dvh] overflow-auto">
            <UserForm
              ref={formRef}
              mode={crud.modalMode}
              initialUser={crud.selectedUser}
              submitting={crud.submitting}
              onSubmit={crud.submitForm}
            />
          </div>

          <div className="flex items-center justify-end gap-2 border-t bg-background px-6 py-4">
            <Button type="button" variant="outline" onClick={crud.closeModal} disabled={crud.submitting}>
              Annuler
            </Button>
            <Button type="button" onClick={() => formRef.current?.submit()} disabled={crud.submitting}>
              {crud.modalMode === "create" ? "Créer" : "Enregistrer"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
    </>
  );
}
