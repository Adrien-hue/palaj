...
export function MonthlyPlanningGrid({
  ...
  const byDate = useMemo(() => new Map(planning.days.map(d => [d.day_date, d])), [planning.days]);
  const allDates = useMemo(() => buildDaysRange(planning.start_date, planning.end_date), [planning.start_date, planning.end_date]);
    ...
    <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
      <div className="grid grid-cols-7 gap-3">
        {["Lun","Mar","Mer","Jeu","Ven","Sam","Dim"].map((d) => (
          <div key={d} className="px-2 text-xs font-semibold text-muted-foreground">{d}</div>
        ))}
