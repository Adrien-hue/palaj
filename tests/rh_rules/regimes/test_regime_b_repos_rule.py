...
from core.rh_rules import RegimeBReposRule
...
class FakeReposAnalyzer:
    ...
    def summarize_workdays(self, work_days):
        # On enregistre le nombre de WD passés pour vérification éventuelle
        self.calls.append(len(work_days))
        return self.summary


# ---------------------------------------------------------------------
# Applicabilité de la règle au régime B
# ---------------------------------------------------------------------


def test_rule_not_applicable_if_regime_not_b():
    """Si regime_id != 0, la règle est non applicable et doit retourner (True, [])."""
    ...
    rule = RegimeBReposRule(analyzer=fake_analyzer) # pyright: ignore[reportArgumentType]

    ok, alerts = rule.check(ctx) # pyright: ignore[reportArgumentType]

    assert ok is True
    assert alerts == []
    # On vérifie que YearRule.check n'a pas été appelé → aucun appel à l'analyzer
...
def test_rule_not_applicable_if_regime_missing():
    ...
    ctx = DummyContext()
    rule = RegimeBReposRule(analyzer=FakeReposAnalyzer(0, 0)) # pyright: ignore[reportArgumentType]
...
def test_empty_year_produces_info_only(make_agent, make_workday):
    ...
    uniquement en 2024 et 2026 :
      ...
      - on doit obtenir une alerte INFO REGIME_B_EMPTY_YEAR pour 2025.
    """
    agent = make_agent(regime_id=0)
    ...
    # start_date = 2024-06-10, end_date = 2026-03-05, donc années 2024, 2025, 2026 analysées.

    rule = RegimeBReposRule(analyzer=FakeReposAnalyzer(0, 0)) # pyright: ignore[reportArgumentType]
    ok, alerts = rule.check(ctx)

    # Résultat global OK (pas d'ERROR)
    assert ok is True
    assert all(a.severity != Severity.ERROR for a in alerts)

    # On doit trouver une synthèse "année vide" pour 2025
    codes = {a.code for a in alerts}
    assert "REGIME_B_EMPTY_YEAR" in codes
    assert any("[2025]" in a.message for a in alerts if a.code == "REGIME_B_EMPTY_YEAR")
...
def test_partial_year_only_info(make_agent, make_workday):
    ...
    fake_analyzer = FakeReposAnalyzer(
        total_rp_days=10,     # largement sous les 114
    ...
    # Année partielle → pas d'erreur bloquante
    assert ok is True
    assert all(a.severity != Severity.ERROR for a in alerts)

    codes = {a.code for a in alerts}
    assert "REGIME_B_REPOS_SYNTHESIS" in codes
    assert "REGIME_B_REPOS_PARTIAL_YEAR" in codes

    # Vérification explicite de la mention "Période incomplète"
...
def test_full_year_with_insufficient_rp_and_sundays(make_agent, make_workday):
    ...
    # Deux jours dans l'année : 01/01 et 31/12 → la fenêtre couvre l'année complète
    ...
    assert ok is False  # erreurs attendues
    codes = {a.code for a in alerts}

    assert "REGIME_B_REPOS_SYNTHESIS" in codes
    assert "REGIME_B_REPOS_RP_INSUFFISANTS" in codes
    assert "REGIME_B_REPOS_DIMANCHES_INSUFFISANTS" in codes

    # On vérifie qu'il y a bien des alertes d'erreur
...
def test_full_year_with_sufficient_rp_and_sundays(make_agent, make_workday):
    ...
    codes = {a.code for a in alerts}
    assert "REGIME_B_REPOS_SYNTHESIS" in codes
    assert "REGIME_B_REPOS_RP_INSUFFISANTS" not in codes
    assert "REGIME_B_REPOS_DIMANCHES_INSUFFISANTS" not in codes
...
def test_no_workdays_returns_year_dates_missing_error(make_agent):
    ...
    → on obtient une erreur YEAR_DATES_MISSING.
    """
    agent = make_agent(regime_id=0)
    ctx = PlanningContext(
        agent=agent,
