...
from core.rh_rules import ReposDoubleRule
...
def test_repos_double_ok(make_context):
    """✅ Cas conforme : GPT de 6 jours suivie de 2 jours de repos."""
    context = make_context(pattern=["POSTE", "POSTE", "POSTE", "POSTE", "POSTE", "POSTE", "REPOS"])
...
def test_repos_triple_ok(make_context):
    """✅ GPT de 6 jours suivie de 3 jours de repos : conforme."""
    context = make_context(pattern=["POSTE", "POSTE", "POSTE", "POSTE", "POSTE", "POSTE", "REPOS", "REPOS"])
...
def test_repos_double_insuffisant(make_context):
    """⚠️ GPT 6 jours suivie d’1 seul jour de repos → alerte attendue."""
    context = make_context(pattern=["POSTE", "POSTE", "POSTE", "POSTE", "POSTE", "POSTE", "REPOS", "POSTE"])
...
def test_pas_de_gpt_6_jours(make_context):
...
def test_gpt_fin_planning(make_context):
    """🕐 GPT de 6 jours à la fin du planning → non évaluable → pas d'alerte."""
    context = make_context(pattern=["POSTE", "POSTE", "POSTE", "POSTE", "POSTE", "POSTE", "REPOS"], include_right_repos=False)
...
def test_aucune_gpt(make_context):
    """✅ Aucun jour travaillé (repos uniquement) → rien à signaler."""
    context = make_context(pattern=["REPOS"] * 5)
    rule = ReposDoubleRule()
    ok, alerts = rule.check(context)
